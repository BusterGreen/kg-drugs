...
-- Callbacks
KGCore.Functions.CreateCallback('kg-drugs:server:RequestConfig', function(_, cb)
    cb(Config.Dealers)
end)

-- Events
RegisterNetEvent('kg-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    local src = source
    ...
    if Config.Dealers[dealer]['products'][itemData.slot].amount - 1 >= 0 then
        ...
        TriggerClientEvent('kg-drugs:client:setDealerItems', -1, itemData, amount, dealer)
    else
        exports['kg-inventory']:RemoveItem(src, itemData.name, amount, false, 'kg-drugs:server:updateDealerItems')
        Player.Functions.AddMoney('cash', amount * Config.Dealers[dealer]['products'][itemData.slot].price, 'kg-drugs:server:updateDealerItems')
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.item_unavailable'), 'error')
    end
end)

RegisterNetEvent('kg-drugs:server:giveDeliveryItems', function(deliveryData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
    local item = Config.DeliveryItems[deliveryData.item].item
    if not item then return end
    exports['kg-inventory']:AddItem(src, item, deliveryData.amount, false, false, 'kg-drugs:server:giveDeliveryItems')
    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items[item], 'add')
end)

RegisterNetEvent('kg-drugs:server:successDelivery', function(deliveryData, inTime)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
    local item = Config.DeliveryItems[deliveryData.item].item
    ...
    local copsOnline = KGCore.Functions.GetDutyCount('police')
    local invItem = Player.Functions.GetItemByName(item)
    if inTime then
        if invItem and invItem.amount >= itemAmount then -- on time correct amount
            exports['kg-inventory']:RemoveItem(src, item, itemAmount, false, 'kg-drugs:server:successDelivery')
            if copsOnline > 0 then
                ...
                if Config.UseMarkedBills then
                    ...
                    exports['kg-inventory']:AddItem(src, 'markedbills', 1, false, info, 'kg-drugs:server:successDelivery')
                else
                    Player.Functions.AddMoney('cash', math.floor(payout * copModifier), 'kg-drugs:server:successDelivery')
                end
            else
                if Config.UseMarkedBills then
                    ...
                    Player.Functions.AddMoney('cash', payout, 'kg-drugs:server:successDelivery')
                end
            end
            TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items[item], 'remove')
            TriggerClientEvent('KGCore:Notify', src, Lang:t('success.order_delivered'), 'success')
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('kg-drugs:client:sendDeliveryMail', src, 'perfect', deliveryData)
                Player.Functions.AddRep('dealer', Config.DeliveryRepGain)
            end)
        else
            TriggerClientEvent('KGCore:Notify', src, Lang:t('error.order_not_right'), 'error') -- on time incorrect amount
            if invItem then
                local newItemAmount = invItem.amount
                local modifiedPayout = deliveryData.itemData.payout * newItemAmount
                exports['kg-inventory']:RemoveItem(src, item, newItemAmount, false, 'kg-drugs:server:successDelivery')
                TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items[item], 'remove')
                Player.Functions.AddMoney('cash', math.floor(modifiedPayout / Config.WrongAmountFee), 'kg-drugs:server:successDelivery')
            end
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('kg-drugs:client:sendDeliveryMail', src, 'bad', deliveryData)
        ...
        if invItem and invItem.amount >= itemAmount then -- late correct amount
            TriggerClientEvent('KGCore:Notify', src, Lang:t('error.too_late'), 'error')
            exports['kg-inventory']:RemoveItem(src, item, itemAmount, false, 'kg-drugs:server:successDelivery')
            Player.Functions.AddMoney('cash', math.floor(payout / Config.OverdueDeliveryFee), 'kg-drugs:server:successDelivery')
            TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items[item], 'remove')
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('kg-drugs:client:sendDeliveryMail', src, 'late', deliveryData)
                Player.Functions.RemoveRep('dealer', Config.DeliveryRepLoss)
            end)
        else
            if invItem then -- late incorrect amount
                ...
                TriggerClientEvent('KGCore:Notify', src, Lang:t('error.too_late'), 'error')
                exports['kg-inventory']:RemoveItem(src, item, itemAmount, false, 'kg-drugs:server:successDelivery')
                Player.Functions.AddMoney('cash', math.floor(modifiedPayout / Config.OverdueDeliveryFee), 'kg-drugs:server:successDelivery')
                TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items[item], 'remove')
                SetTimeout(math.random(5000, 10000), function()
                    TriggerClientEvent('kg-drugs:client:sendDeliveryMail', src, 'late', deliveryData)
                    Player.Functions.RemoveRep('dealer', Config.DeliveryRepLoss)
...
RegisterNetEvent('kg-drugs:server:dealerShop', function(currentDealer)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    exports['kg-inventory']:CreateShop({
    ...
    exports['kg-inventory']:OpenShop(src, dealerData.name)
end)

-- Commands

KGCore.Commands.Add('newdealer', Lang:t('info.newdealer_command_desc'), { {
    ...
    if result then return TriggerClientEvent('KGCore:Notify', source, Lang:t('error.dealer_already_exists'), 'error') end
    MySQL.insert('INSERT INTO dealers (name, coords, time, createdby) VALUES (?, ?, ?, ?)', { dealerName, pos, time, Player.PlayerData.citizenid }, function()
        ...
        TriggerClientEvent('kg-drugs:client:RefreshDealers', -1, Config.Dealers)
    end)
end, 'admin')

KGCore.Commands.Add('deletedealer', Lang:t('info.deletedealer_command_desc'), { {
    ...
    if result then
        ...
        TriggerClientEvent('kg-drugs:client:RefreshDealers', -1, Config.Dealers)
        TriggerClientEvent('KGCore:Notify', source, Lang:t('success.dealer_deleted', { dealerName = dealerName }), 'success')
    else
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.dealer_not_exists_command', { dealerName = dealerName }), 'error')
    end
end, 'admin')

KGCore.Commands.Add('dealers', Lang:t('info.dealers_command_desc'), {}, false, function(source, _)
    local DealersText = ''
    if Config.Dealers ~= nil and next(Config.Dealers) ~= nil then
        ...
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.no_dealers'), 'error')
    end
end, 'admin')

KGCore.Commands.Add('dealergoto', Lang:t('info.dealergoto_command_desc'), { {
    ...
    if Config.Dealers[DealerName] then
        ...
        TriggerClientEvent('KGCore:Notify', source, Lang:t('success.teleported_to_dealer', { dealerName = DealerName }), 'success')
    else
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.dealer_not_exists'), 'error')
    end
end, 'admin')
